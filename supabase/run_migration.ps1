# Migration runner - splits the SQL into chunks and applies each via API
...
# Chunk 1: Drop + Create static tables
$chunk1 = @"
...
# Chunk 7: Seed schedule + preorder items
$chunk7 = @"
...
insert into site_settings (id, site_meta, hero, pricing, opening_hours, admin_config, pass_faq, promo) values (
  ...
  '[{"q":"How do I use my pass?","a":"Book any night as usual, then tick Use my Visit Pass on the form. We will automatically redeem one visit from your pass when you attend."},{"q":"Do passes expire?","a":"Yes — 5-visit passes are valid 90 days, 10-visit 180 days, 20-visit 365 days from purchase."},{"q":"Can I share my pass?","a":"Passes are tied to one email. You can bring a friend on the night but only your seat uses a visit."},{"q":"What if I cant make a night?","a":"Cancel up to 24 hours before for free. Cancellations inside 24 hours do not refund the visit."},{"q":"Refunds?","a":"Unstarted passes are refundable within 14 days. Once a visit is used the pass is non-refundable."}]'::jsonb,
...
# Run all chunks
$ok = $true
$ok = (Run-SqlChunk "1. Drop + static tables" $chunk1) -and $ok
$ok = (Run-SqlChunk "2. Dynamic tables" $chunk2) -and $ok
$ok = (Run-SqlChunk "3. Functions" $chunk3) -and $ok
$ok = (Run-SqlChunk "4. RLS + Indexes" $chunk4) -and $ok
$ok = (Run-SqlChunk "5. Seed pass_templates" $chunk5) -and $ok
$ok = (Run-SqlChunk "6. Seed products" $chunk6) -and $ok
$ok = (Run-SqlChunk "7. Seed schedule + preorder_items" $chunk7) -and $ok
$ok = (Run-SqlChunk "8. Seed site_settings" $chunk8) -and $ok

if ($ok) {
    Write-Host "`n=== ALL CHUNKS OK ===" -ForegroundColor Green
} else {
    Write-Host "`n=== SOME CHUNKS FAILED ===" -ForegroundColor Red
}
