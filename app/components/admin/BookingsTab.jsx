...
const getTicketAuditText = (booking) => {
  const ticketText = getTicketText(booking)
  const ticketId = booking?.user_ticket_id || booking?.ticket_id || ''
  if (ticketText && ticketId) return `${ticketText}（套票 #${ticketId}，已扣 1 次）`
  if (ticketText) return `${ticketText}（已扣 1 次）`
  if (ticketId) return `套票 #${ticketId}（已扣 1 次）`
  return ''
}
