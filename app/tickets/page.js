...
const getPurchaseMessage = (response, payload, ticketName) => {
  if (payload?.paymentProvider === 'stripe' || payload?.checkoutUrl) {
    return `正在前往 Stripe 付款頁；付款成功後，${ticketName} 會自動加入你的會員帳戶。`
  }
...
export default function TicketsPage() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      setPaymentMethod(nextSettings.stripe_enabled === 'false' && nextSettings.manual_payment_enabled !== 'false' ? 'manual' : 'stripe')
      setLoading(false)
    }
  ...
  const paymentOptions = useMemo(() => {
    const options = []
    if (settings.stripe_enabled !== 'false') options.push({ value: 'stripe', label: 'Stripe 線上付款' })
  ...
  return (
    <>
      <section className="vh-page-hero">
        ...
        <p>購買套票並完成付款後，套票會顯示在會員中心。預約適用服務時可扣 1 次使用，取消後會回補。</p>
      </section>
