...
USING (
  EXISTS (
    ...
    WHERE mp.id = auth.uid() AND mp.is_admin = true
  )
);
...
-- Allow authenticated members to create their own order and ticket records.
...
-- Shared admin check reused across business tables.
...
-- Example:
-- UPDATE public.member_profiles SET is_admin = true WHERE email = 'you@example.com';
